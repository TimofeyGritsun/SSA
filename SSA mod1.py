...
def sort_groups(groups, invariants):
    def group_key(group):
        size = len(group)
        sig = poly_vector_signature(invariants[group[0]])
        return (size, sig)
...
def iterative_refinement_with_ordering(G, hull, max_iterations=5):
    ...
    groups = sort_groups(groups, base_invariants)
    print_groups_with_polynomials(
        groups,
        base_invariants,
        title="ШАГ 1: базовые инварианты (2 полинома)"
    )
    ...
    if not any(len(g) == 1 for g in groups):
        print("\nНЕТ ОПОРНОЙ ПОЗИЦИИ")
        return {p: base_invariants[p] for p in base_invariants}
    ...
    for g in groups:
        if len(g) == 1:
            p = g[0]
            frozen.add(p)
            current_invariants[p] = base_invariants[p]

    ordered_positions = [p for g in groups for p in g]
    ...
    while iteration <= max_iterations:
        ...
        extended_inv = extend_invariants_with_pivot(
            G,
            hull,
            pivot,
            frozen,
        ...
        for g in groups:
            ...
            sub = {}
            for p in g:
                sub.setdefault(extended_inv[p], []).append(p)

            subgroups = list(sub.values())
            subgroups = sort_groups(subgroups, extended_inv)
        ...
        for g in groups:
            if len(g) == 1:
                ...
                if p not in frozen:
                    frozen.add(p)
                    current_invariants[p] = extended_inv[p]
    ...
    for g in groups:
        for p in g:
            if p in current_invariants:
                final_invariants[p] = current_invariants[p]
            else:
                final_invariants[p] = extended_inv[p]
...
res = 0
for i in range(it):
    res += get_res(hull, n, k, choice=None)
