...
def iterative_weight_refinement(G, max_pivot_size=8):
    n = len(G[0])
    ...
    singleton_order = [g[0] for g in groups if len(g) == 1]

    if max_pivot_size is None:
        max_pivot_size = len(singleton_order)

    step = 1

    for k in range(1, max_pivot_size + 1):

        for pivots in combinations(singleton_order, k):

            print(f"ШАГ {step + 1}, опорные позиции: {[p+1 for p in pivots]}")
            ...
            if all(len(g) == 1 for g in groups):
                print("Все позиции стали уникальными.\n")
                return current_inv
...
def match_positions_by_invariants(G, G1):
    inv_G  = iterative_weight_refinement(G, 3)
    inv_G1 = iterative_weight_refinement(G1, 3)
